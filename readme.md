...
Continuous integration is a way to **build software, automaticly run tests and alert developers when a test doesn't pass**. This way developers can fix mistakes right away before they pile up and become difficult to manage. It aslo gives developers more confidance in their code since developers are notified of issues as they come and minimizes unknown errors. 

### Getting started with Github Actions
Simply navigate to your repository and click the **Actions** on the right side of the *Pull request* button.
Here you can choose to use actions from others or click **set up a workflow yourself** to create your own.

#### On event
Your workflow should run at minimum each time a push or a pull request is made to the master. Another event to keep in mind is [schedule](https://help.github.com/en/actions/reference/events-that-trigger-workflows#scheduled-events-schedule) which runs an action on scheduled times. If actions take a long time to run it is advisable to run them over night. 

To see a full list of possible events click [here](https://help.github.com/en/actions/reference/events-that-trigger-workflows#webhook-events) and to see more details on the keyword on click [here](https://help.github.com/en/actions/reference/workflow-syntax-for-github-actions#on).

#### Jobs
Jobs should include testing the system. See more on testing in the [testing section](https://github.com/cadia-lvl/SoftwareDevelopmentGuidelines#testing).
Multiple jobs can be specified within each workflow. Each job may take no longer than 6 hours to compleate or it will be automaticly ended.
...
More details on jobs can be found [here](https://help.github.com/en/actions/reference/workflow-syntax-for-github-actions#jobs).
...
job: 
  # A unique id for a job containing only _ or letters
  name_of_first_job:
    # Specifies which kind of machine the job is run on. 
    # It's up to you which machine kind to use but github hosted runners are highly encouraged.
    runs-on: ubuntu-latest
    
    #steps defines a tasks to run as part of the job, the following steps 
    steps: 
      # This allowes you to use an action declaired in another file, also works for test files
      ...
      - name: one line script example
        run: echo this is a one line script
        
      # or a multiline one
      - name: this is a multiline script
        run: |
        echo this is line one
        echo and this is line two
        echo there could be even more!
```
#### More information
